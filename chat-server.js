...
wsServer.on('request', function(request) {
  ...
  if (history.length > 0) {
    ...
    var  historys= JSON.stringify(history);
    for(var i = 0;i<history.length;i++){
        var content = JSON.parse(JSON.stringify(history[i]));
       if(content.type==='Buffer'){
        ...
        connection.sendBytes(buf);
            ...
            connection.sendUTF(
         JSON.stringify({ type: 'history',data: [content]} ));
       }
    }
    //connection.sendUTF(
  ...
  connection.on('message', function(message) {
    ...
    // first message sent by user is their name
     ...
     if (userName === false) {
        // remember user name
        userName = htmlEntities(content.msg);
